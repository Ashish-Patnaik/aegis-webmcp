...
export default function AegisDashboard() {
  ...
  // Listen for the Native Script events
  useEffect(() => {
  ...
  return (
    <>
      {/* 
        THE NATIVE WEBMCP INJECTOR
        This executes at 0ms, completely bypassing Next.js hydration issues.
        It creates the exact spec GoogleChromeLabs expects.
      */}
      <script dangerouslySetInnerHTML={{ __html: `
        ...
        document.modelContext.registerTool({
          ...
          execute: async (input) => {
            window.dispatchEvent(new CustomEvent('aegis-agent-action', { detail: { type: 'UNIT_STATUS', data: input } }));
            return { status: "success" };
          }
        });
        ...
        document.modelContext.registerTool({
          ...
          execute: async (input) => {
            window.dispatchEvent(new CustomEvent('aegis-agent-action', { detail: { type: 'ALERT', data: input } }));
            return { status: "success" };
          }
        });
      `}} />
